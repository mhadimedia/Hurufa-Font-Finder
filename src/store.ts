...
// Define a type for the persisted font metadata
interface PersistedFontData {
  tags?: string[];
  language?: string;
}
...
interface StoreState {
  ...
  selectFontsByIds: (fontIds: string[], select: boolean) => void;
...
// Define the persisted part of the state
interface PersistedState {
  ...
  lineHeight: number;
  letterSpacing: number;
  persistedFontData: Record<string, PersistedFontData>; // Map font ID to tags/language
}

// Wrap the store creation with persist
export const useFontStore = create<StoreState>()(
  persist(
    (set, get) => {
      ...
      return {
        ...
        // Actions
        setFonts: (fonts) => {
          const { persistedFontData } = get() as any as { persistedFontData: Record<string, PersistedFontData> };
          // Merge persisted tags/language back onto the newly loaded fonts
          const mergedFonts = fonts.map(font => {
            const fontId = font.postscriptName || font.fullName;
            const persistedData = persistedFontData?.[fontId];
            return {
              ...font,
              tags: persistedData?.tags || [],
              language: persistedData?.language || font.language, // Keep detected if none saved
            };
          });
          set({ fonts: mergedFonts });
        },
        ...
        selectFontsByIds: (fontIds, select) => {
          ...
          // Note: We don't update lastSelectedFont here as it's a bulk action
          set({ selectedFonts: newSelected });
        },
        moveSelectedFonts: (category) => {
          const { fonts, selectedFonts } = get();
          let newPersistedData: Record<string, PersistedFontData> | null = null;
          const updatedFonts = fonts.map(font => {
            ...
            if (selectedFonts.has(fontId)) {
              ...
              // Lazily create the persisted data update object
              if (!newPersistedData) newPersistedData = { ...(get() as any as { persistedFontData: Record<string, PersistedFontData> }).persistedFontData };
              newPersistedData[fontId] = { ...newPersistedData[fontId], tags };
              return { ...font, tags };
            }
            return font;
          });
          set({ 
            fonts: updatedFonts, 
            fontCategories: categorizeFonts(updatedFonts),
            ...(newPersistedData && { persistedFontData: newPersistedData })
          });
        },
        bulkUpdateTags: (tagsToAdd, tagsToRemove) => {
          ...
          const updatedFonts = fonts.map(font => {
            ...
            if (selectedFonts.has(fontId)) {
              ...
              tagsToAdd.forEach(tag => { if (!tags.includes(tag)) tags.push(tag); });
              // Lazily create the persisted data update object
              if (!newPersistedData) newPersistedData = { ...(get() as any as { persistedFontData: Record<string, PersistedFontData> }).persistedFontData };
              newPersistedData[fontId] = { ...newPersistedData[fontId], tags };
              return { ...font, tags };
        ...
        bulkSetLanguage: (language) => {
          ...
          const updatedFonts = fonts.map(font => {
            ...
            if (selectedFonts.has(fontId)) {
              // Lazily create the persisted data update object
              if (!newPersistedData) newPersistedData = { ...(get() as any as { persistedFontData: Record<string, PersistedFontData> }).persistedFontData };
              newPersistedData[fontId] = { ...newPersistedData[fontId], language };
    ...
    {
      name: 'font-organizer-storage', // Name for storage (e.g., localStorage key)
      // Use default localStorage for now
      ...
      partialize: (state): PersistedState => ({
        ...
        // Create the map of fontId -> {tags, language}
        persistedFontData: state.fonts.reduce((acc, font) => {
          const fontId = font.postscriptName || font.fullName;
          if (font.tags?.length || font.language) {
            acc[fontId] = {};
            if (font.tags?.length) acc[fontId].tags = font.tags;
            if (font.language) acc[fontId].language = font.language;
          }
          return acc;
        }, {} as Record<string, PersistedFontData>)
      }),
      // Custom merge function to handle merging persistedFontData back into fonts array later
      merge: (persistedStateUnknown, currentState) => {
        const persistedState = persistedStateUnknown as PersistedState;
        // Keep the loaded fonts array, we'll merge into it in setFonts
        return {
          ...currentState,
          ...persistedState,
          fonts: currentState.fonts, // Keep initially empty fonts array
        };
      },
    }
  )
